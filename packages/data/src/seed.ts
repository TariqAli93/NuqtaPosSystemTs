...
const initializeDatabase = async (input: InitializeDatabaseInput): Promise<void> => {
  ...
  const accountsData = [
    ...
    { code: '1300', name: 'ضريبة المدخلات', accountType: 'asset', parentId: null },
    { code: '2100', name: 'ذمم الموردين', accountType: 'liability', parentId: null },
    { code: '2200', name: 'ضريبة المخرجات', accountType: 'liability', parentId: null },
  ...
  async function seedPreset(presetKey: PresetKey): Promise<SeedCounters> {
    ...
    for (const prod of preset.products) {
      ...
      const existingBatches = productRepo.findBatchesByProductId(prodInfo.id);
      ...
      try {
        ...
        console.warn(`    ⚠ Could not create opening stock for '${prod.sku}': ${msg}`);
      }
    }
  ...
  console.log('   • 8 chart of accounts (for Balance Sheet, P&L, Trial Balance reports)');
  console.log('');
