...
export const customers = sqliteTable('customers', {
  ...
  createdAt: text('created_at').default(sql`(datetime('now','localtime'))`),
  updatedAt: text('updated_at').default(sql`(datetime('now','localtime'))`),
  createdBy: integer('created_by'), // Intentionally removed FK constraints for strict separation simplicity for now, or keep references to IDs
});
...
// Audit Logs Table
// Immutable record of critical business actions for compliance & forensics
export const auditLogs = sqliteTable('audit_logs', {
  id: integer('id').primaryKey({ autoIncrement: true }),
  userId: integer('user_id').notNull(),
  action: text('action').notNull(), // e.g., 'sales:create', 'products:update', 'user:role-change'
  entityType: text('entity_type').notNull(), // e.g., 'Sale', 'Product', 'User'
  entityId: integer('entity_id').notNull(),
  timestamp: text('timestamp')
    .notNull()
    .default(sql`(datetime('now','localtime'))`),
  changedFields: text('changed_fields'), // JSON: { "fieldName": { "old": value, "new": value }, ... }
  changeDescription: text('change_description'), // Human-readable summary
