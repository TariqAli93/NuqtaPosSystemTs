...
export class ReconcileSupplierBalanceUseCase {
  ...
  async execute(): Promise<ReconcileSupplierBalanceResult> {
    const { items: suppliers } = await this.supplierRepo.findAll({ limit: 100000, offset: 0 });
    ...
    for (const supplier of suppliers) {
      ...
      const ledgerBalance = this.supplierLedgerRepo.getLastBalanceSync(supplier.id);
  ...
  async repair(): Promise<number> {
    ...
    for (const item of driftItems) {
      await this.supplierRepo.update(item.supplierId, { currentBalance: item.ledgerBalance });
    }
