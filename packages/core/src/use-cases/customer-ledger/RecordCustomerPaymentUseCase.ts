...
export class RecordCustomerPaymentUseCase {
  ...
  executeCommitPhase(data: RecordPaymentInput, userId: number): CustomerLedgerEntry {
    ...
    const balanceBefore = this.ledgerRepo.getLastBalanceSync(data.customerId);
  ...
  private createJournalEntry(paymentId: number, amount: number, userId: number): void {
    ...
    if (!cashAcct?.id || !arAcct?.id) {
      console.warn('[RecordCustomerPaymentUseCase] Missing cash/AR accounts, skipping journal');
      return;
    }
    ...
    this.accountingRepo.createJournalEntrySync({
      ...
      createdBy: userId,
      lines: [
        { accountId: cashAcct.id, debit: amount, credit: 0, description: 'Cash received' },
        { accountId: arAcct.id, debit: 0, credit: amount, description: 'AR settled' },
      ],
    });
  }
