...
import { InvalidStateError, ValidationError } from '../../errors/DomainErrors.js';
...
export class PostPeriodUseCase {
  ...
  execute(input: PostPeriodInput, userId: number): PostingBatch {
    ...
    if (accountingEnabled === 'false') {
      throw new InvalidStateError('Accounting is not enabled. Cannot post entries.');
    }
    ...
    if (input.periodStart > input.periodEnd) {
      throw new ValidationError('Period start must be before or equal to period end');
    }

    // Get unposted entries for the period
    const entries = this.postingRepo.getUnpostedEntries(input.periodStart, input.periodEnd);

    if (entries.length === 0) {
      throw new InvalidStateError('No unposted entries found for the specified period', {
    ...
    const batch = this.postingRepo.createBatch({
    ...
    this.postingRepo.markEntriesAsPosted(entryIds, batch.id!);
